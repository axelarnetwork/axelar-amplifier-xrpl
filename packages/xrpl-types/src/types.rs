...
pub mod tx_hash_hex {
    ...
    where
        S: Serializer,
    {
        HexBinary::from(value.tx_hash).serialize(serializer)
    }
    ...
    where
        D: Deserializer<'de>,
    {
        let hex_binary = HexBinary::deserialize(deserializer)?;
        Ok(HexTxHash {
            tx_hash: hex_binary.as_slice().try_into().map_err(Error::custom)?,
        })
    }
}
