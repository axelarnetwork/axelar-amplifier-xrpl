...
use crate::types::{tx_hash_hex, xrpl_account_id_string, XRPLAccountId, XRPLPaymentAmount};
...
impl From<XRPLUserMessage> for Vec<Attribute> {
    fn from(other: XRPLUserMessage) -> Self {
        let mut array = vec![
            ("tx_id", HexBinary::from(other.tx_id.tx_hash).to_string()).into(),
...
impl From<XRPLProverMessage> for Vec<Attribute> {
    fn from(other: XRPLProverMessage) -> Self {
        vec![
            ...
            (
                "unsigned_tx_hash",
                HexBinary::from(other.unsigned_tx_hash.tx_hash).to_string(),
            )
                .into(),
        ]
    }
