...
use crate::handlers::ton_verify_msg::{TonClient, Message, FetchingError};
use axelar_wasm_std::msg_id::HexTxHash;

use tonlib_core::{
    cell::{Cell, CellBuilder, CellParser, TonCellError}
};
...
fn parse_call_contract_log(message_id: HexTxHash, cell: &Arc<Cell>) -> Result<Message, Error>{
    ...
    let payload_hash: [u8; 32] = parser.load_bits(256)?.try_into().map_err(|e| anyhow!("Conversion failed"))?;

    return Ok(Message { message_id, payload_hash: H256::from(payload_hash), destination_address, destination_chain: ChainName::from_str(&destination_chain)?, source_address })
}
...
impl TonClient for MockTonClient {
    async fn get_tx(&self, _tx_hash: &HexTxHash, _gateway: &TonAddress) -> error_stack::Result<Message, FetchingError> {
        // Always return Some("success") to simulate a successful transaction
        Ok(Message{ 
            message_id: HexTxHash::from_str("949b738e28e46ca279bd339f6967f973f4e1f8f025252be8222c97e115d24e6d".into()).unwrap(), 
            ...
            source_address: "0QCJitE8BZ8qOmlXagEMQa8jm0h7xVXqvrmliU3rESmTMCkR".parse().unwrap(), 
            payload_hash: H256::from_str("72a1d814ac3bc3f32c851b71e1189910e10506aaabf6416303f84d385a736493").unwrap() 
        })
    }
}
...
use std::{any, collections::{hash_map::Iter, HashMap}};
...
impl TonClient for TonRpcClient {
    async fn get_tx(&self, tx_hash: &HexTxHash, gateway: &TonAddress) -> error_stack::Result<Message, FetchingError> {
        ...
        //let gateway: TonAddress = "EQDdF2mqsiJ1lj-fTO-uRUa3qvUqdRUp0FGNMeLKXZTPnOKR".parse()?;

        let client = Client::new();

        let res = client.get(&format!("{}/{}", self.rpc_url, method)).query(&data)
            .send().await.change_context(FetchingError::Client)?;
        ...
        if let Some(address) = result.get("transactions")
            .and_then(|v| v.get(0))
        ...
        if let Some(aborted) = result.get("transactions")
            .and_then(|v| v.get(0))
        ...
        if let Some(bounced) = result.get("transactions")
            .and_then(|v| v.get(0))
        ...
        if let Some(opcode) = result.get("transactions")
            .and_then(|v| v.get(0))
            .and_then(|v| v.get("in_msg"))
        ...
        if let Some(log) = result.get("transactions")
            .and_then(|v| v.get(0))
